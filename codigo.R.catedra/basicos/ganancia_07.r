...
for(  vmaxdepth  in  3:12 )
{
	...
	cat( format(Sys.time(), "%Y%m%d %H%M%S"), "d:\\uba\\a2016\\producto_premium_201604.txt", "rpart", vcp , vminsplit, vminbucket, vmaxdepth, mean(ganancias), mean(tiempos), ganancias , "\n", sep="\t", file="d:\\uba\\a2016\\salida.txt", fill=FALSE, append=TRUE )
