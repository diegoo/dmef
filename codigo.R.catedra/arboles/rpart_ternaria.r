...
# source( "d:\\uba\\a2016\\codigoR\\rpart_ternaria.r" )
...
archivo_salida  <- "d:\\uba\\a2016\\salida_ternaria.txt"
...
for(  vmaxdepth  in  c(  4, 5, 6, 7,8,9,10, 11, 12 ) )
{
	...
	for( s in  1:5 )
	{
		...
		t0 =  Sys.time()
		abril_modelo  <- rpart( clase ~ .   ,   data = abril_dataset_training,  method="class", xval=0, maxsurrogate=1, surrogatestyle=1,   cp=vcp, minsplit=vminsplit, minbucket=vminbucket, maxdepth=vmaxdepth )  
		t1 = Sys.time()
		...
		ganancias[s] = ganancia( abril_testing_prediccion,  abril_dataset_testing$clase,  (250/8000)  ) / 0.30
	...
	for( s in  1:5 )
	{
		...
		# generacion del modelo sobre los datos de training
		t0 =  Sys.time()
		abril_modelo  <- rpart( clase ~ .   ,   data = abril_dataset_training,  weights=vweights,  method="class", xval=0, maxsurrogate=1, surrogatestyle=1,   cp=vcp, minsplit=vminsplit, minbucket=vminbucket, maxdepth=vmaxdepth )  
		t1 = Sys.time()
		tiempos[s] <-  as.numeric(  t1 - t0, units = "secs")
		...
		# calculo la ganancia normalizada  en testing
		ganancias[s] = ganancia( abril_testing_prediccion,  abril_dataset_testing$clase,  0.5 ) / 0.30
