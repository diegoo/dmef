...
archivo.salida <- paste("prediction", semilla, ntree.limit, ".txt")
...
stopifnot(nrows(dataset) == 152832)
stopifnot(rownames(dataset)[1] == 1267605)
stopifnot(rownames(dataset)[length(rownames(dataset))] == 1000461)
stopifnot(length(rownames(dataset)) == nrows(dataset))
stopifnot(length(rownames(dataset)) == length(prediction) / 2)
ids <- rownames(dataset)

model <- xgb.load(model.name)
prediction <- predict(model, dataset.m, ntreelimit = ntree.limit)
predicted.ids <- get.ids(prediction, ids, probability.threshold)
